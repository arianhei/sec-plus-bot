...
module.exports = {
    name: 'gr',
    description: 'Give role of member or everyone <a:tik3:777128127804735518>',
    execute(message) {
        const db = require('../db')
        ...
        delete db;
        if (args.length === 1) {
            return message.channel.send({ embed: { title: "<a:qal:777127378253774858> Error 0x504: you forgot to mention member", description: `Ex: ${json_log.PREFIX}rg \`@everyone\` \`@role\` ` } })
        }
        if (args[1] === "all" || args[1] === "@everyone") {
            const ro = message.mentions.roles
            if (ro === null) {
                return message.channel.send({ embed: {title: "<a:qal:777127378253774858> ERROR 0x505: you forgot to mention role!", description: `Ex: ${json_log.PREFIX}rg \`@everyone\` \`@role\``} })
            }
            ro.forEach(ro => {
                if(ro.managed){
            ...
            if (r.first() === undefined) {
                return message.channel.send({embed: {title: "<a:qal:777127378253774858> Error 0x505: you forgot to mention role!", description: `Ex: ${json_log.PREFIX}gr \`@everyone\` \`@role\``}})
            }
